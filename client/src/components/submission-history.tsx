      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-text">Failed to load submission history</p>
            <p className="text-sm text-text opacity-75">{(error as Error).message || "Unknown error"}</p>
          </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-text">No submission history</p>
            <p className="text-sm text-text opacity-75">Your recent requests will appear here</p>
          </div>
        ) : (
            ...
            {recentSubmissions.map((submission) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-medium text-accent text-sm">{submission.requesterName}</h3>
                    <p className="text-xs text-text">{countInstruments(submission)} instruments selected</p>
                  </div>
